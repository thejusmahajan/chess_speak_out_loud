...
**0. `briefs/2026-09-09_idm-clinical-ai-research-german-and-interview-prep.md` — ⛑ ACTIVE. Execute this one FIRST.** IDM gGmbH, Hamburg — AI/ML Engineer, Clinical AI. Research the employer (**every claim needs a URL**), decide German-vs-English **from evidence**, draft the German letter if warranted, and build interview prep anchored in his real record. **⚠ This is the exact brief shape that produced three fabricated strategy documents on 2026-09-07** — an invented reference number, a wrong portal limit, and a table telling him to relabel his CV with skills he does not have. **Nothing may be added to his CV or letter that he cannot defend**, and the letter's honest gap paragraph must survive translation intact. Two files may be created; **no `.tex` or `.pdf` may be edited**.

> **⛑ BEFORE TOUCHING THE HEREON 1059 CV**, read
> `applications/hereon_agentic_ai_1059/CV_CHANGELOG.md`. That CV uses a **LOCAL, modified**
> `thejus_cv.cls` whose `\cvheader` takes **7 arguments** (the 1056 one takes 6), the
> photograph has been deliberately removed, and **every `.tex` must be compiled TWICE** or the
> header renders completely blank with no error. The changelog documents every change, the
> accuracy corrections that must not be silently restored (Linux cluster / Colab GPUs, **not**
> university HPC), and exact revert instructions including how to put the photo back.

**0. `briefs/2026-09-07_publication-list-independent-verification.md` — ⛑ ACTIVE. Execute this one
FIRST.** Read-only audit, web access required, ~45 min. A full publication list went onto Thejus's
CV today for a Hereon application being sent **today**; the leader assembled it and it has not been
independently checked. **The brief contains a sealed Appendix A — do not open it before Step 5**, or
the audit degrades into a proofread. Two citations on the live website are already known bad; the
base rate of error here is not zero. *This is an application/deadline item and does not consume the
one non-interview ACTIVE slot.*

**0. `briefs/2026-09-09_publish-jax-port-resume-after-checkpoint-1.md` — ⛑ ACTIVE.** Resumes the JAX
port packaging after the Checkpoint 1 stop, which is **AUDITED ACCEPT**. All exclusion decisions are
already made in §0 — do not re-litigate them. ⚠ **Correction carried into it:** `medwed.txt` is
**CC BY 4.0 open access**, so redistribution would be legal; it is excluded on hygiene (a code repo
should not ship paper texts, and that copy carries an institutional download banner), *not* on
copyright. Only `beckmann.txt` (Elsevier, all rights reserved) is a legal prohibition.

**0z. `briefs/2026-09-09_ecc-harness-study-and-antigravity-mapping.md` — ⛑ ACTIVE. Study only,
...
Two deliveries this week wrote files their brief had placed off limits. Named traps: you are
describing a harness for a system you are not — every claim needs a source; and ECC targets general
software teams, so the **NOT APPLICABLE list must be non-empty**.

**0a. `briefs/2026-09-08_publish-jax-water-column-port.md` — ⛑ ACTIVE (after the 1059 application is
sent; it does not block it).** Prepare Thejus's **JAX port of the Fortran water-column model** for
publication from `downloads/python_port-20260908T155704Z-1-001.zip`. It is the most job-relevant
claim on his CV and is currently **invisible** — the public repo his CV links contains the
*Fortran/OpenMP* model. **Blast-radius EXTERNAL: it becomes a public repo linked from a live job
application.** Named traps: `knowledge/beckmann.txt` and `knowledge/medwed.txt` may be extracted
text from **copyrighted papers** — classify every file before packaging and STOP if so; the code is
~2 years old so expect JAX API drift; **no speedup numbers, no TPU claims** (both notebooks ran on a
**T4 GPU**); and **nothing is published by the worker** — it is prepared locally and Thejus pushes.

**0b. `briefs/2026-08-29_pytorch-certificate-rollout.md` — ⚑ PART A ✅ AUDITED ACCEPT 2026-08-29.
...
|---|---|---|---|---|---|
| *(no brief)* `STRATEGIC_ANALYSIS.md` + `STRATEGIC_BLUEPRINT.md` + `STRATEGIC_PREPRATION.md` | job_search / hereon 1059 | unbriefed research | **AUDITED — ACCEPT WITH CORRECTIONS** | the two files themselves | `reports/2026-09-07_hereon-1059-strategy-documents_AUDIT.md`. Written unprompted into a folder a live brief had declared off-limits; nothing else in the folder was touched. **6 scientific/institutional claims VERIFIED** (GENIUS/arXiv 2512.06404 with **Aydin** a co-author; Creator-Inspector/arXiv 2605.23754 with **Cyron** a co-author, 91%→100% on Opus; MechVivo €10M ERC Synergy; HFMI/HAICORE). **3 operational claims failed, all of which would have touched what we send:** reference `2026/MS 6` **UNSUPPORTED** by either language version of the posting (not used); portal limit `20 MB` **CONTRADICTED** by Hereon's own guide (5 MB total); "CV must feature C++ and pybind11" **rejected** as overclaiming. Citation density: 1 URL in 235 lines, 0 in 253. **All TVöD salary figures are UNCHECKED — do not quote them in a negotiation.** **⚠ The third file (`STRATEGIC_PREPRATION.md`, 35,666 bytes on 2 lines, 0 URLs) is REJECTED on its central recommendation:** a "Lexical Optimization" table telling him to relabel his CV in the hiring group's own vocabulary for systems he has not built (CANNs, Creator-Inspector, AEH, pybind11, Clausius-Duhem) to pass HR screening — i.e. borrow Cyron's and Aydin's terms for their own papers, in front of Cyron and Aydin. It also advises claiming pybind11 as professional experience to win a higher **Stufe**, which is a formal public-sector pay determination. One genuinely good point survives: the EG 13 band is fixed, the **Stufe** is the negotiable axis — and Thejus can argue it from real documented experience (Paris-Saclay 2015–18, UniHamburg post-doc, HealthTwiSt). |
| `2026-09-09_idm-clinical-ai-research-german-and-interview-prep` | job_search / web | research + drafting | **REJECT (2026-09-12)** | leader | **Fabricated the application address.** Asserted *"direct email to `karriere@idmedizin.de`, no portal"* (§41, §63, plus a `careers@` variant). That address appears nowhere on idmedizin.de; the posting prescribes a recruiting portal. It quoted *"Wir sichten jede Bewerbung persönlich"* accurately but dropped the preceding *"Die Bewerbung läuft über unser Recruiting-Portal"*, inverting the meaning — **a real quote carrying a false claim, which the grep-the-quote rule does not catch.** Shipped into the bundle because the report was never audited. Posting now stored at `job_search/applications/idm_clinical_ai/JOB_POSTING.md`. |
| `2026-09-09_publish-jax-port-resume-after-checkpoint-1` | new public repo (prepared locally) | implementation | **AUDITED ACCEPT** | `reports/2026-09-09_publish-jax-port-resume-after-checkpoint-1_REPORT.md` | **ACCEPT — cleanest delivery so far.** Verified from the artefact, not the report: `git ls-files` = exactly 15 intended files, **no remote**, one local commit, nothing pushed. `knowledge/`, `results/`, `__pycache__` absent from the tracked set and in `.gitignore`. It **ran** (JAX 0.10.2, `[CpuDevice(id=0)]`, 21.35 s), hit a real `UnicodeDecodeError` — `config.py:152` opened `config.yaml` without an encoding, and the file contains `°C`, which breaks on Windows cp1252 — and fixed it with explicit UTF-8, declared. README carries **no** speed claims, states T4 GPU sourced to notebook metadata, and explicitly disclaims TPU execution and the absence of an equivalence test suite. Minor: the encoding fix was folded into the initial commit rather than a separate labelled one. **Leader then added GPLv3 + a provenance note** (Thejus's decision). |
| `2026-09-09_ecc-harness-study-and-antigravity-mapping` | chess_speak_out_loud (report only) | research | **AUDITED ACCEPT (one correction)** | `reports/2026-09-09_ecc-harness-study-and-antigravity-mapping_REPORT.md` | **ACCEPT.** Boundary clean, nothing implemented. Cited Antigravity's own bundled docs at `~/.gemini/antigravity-ide/builtin/skills/` — authoritative, and the leader confirmed the paths exist. **Central question ANSWERED YES:** `hooks.md:193` — a `PreToolUse` hook returning `{"decision":"deny"}` **hard-blocks the tool call**; payload shape at `hooks.md:162-214` verified by the leader, so the proposed `gate_scope.py` design is sound. Filter honest: 1 ADOPT, 2 ADAPT, **6 NOT APPLICABLE**. **Correction:** its "Mechanism A" gives a settings syntax `permissionOverrides: ["deny:write_file(...)"]` that the leader could **not** verify — in the docs `permissionOverrides` is a *hook output* field for temporary grants. The *feature* is real (`app.md:55-58` "Permission Grants: define global allow/deny rules for specific files") but that exact key/syntax is unconfirmed. **Use Mechanism B (the hook), not A.** |
| `2026-09-08_publish-jax-water-column-port` | new public repo (prepared locally) | implementation | **DELIVERED — STOPPED AT CHECKPOINT 1, not yet audited** | `reports/2026-09-08_publish-jax-water-column-port_REPORT.md` | Pending leader audit. Headline: `knowledge/beckmann.txt` and `knowledge/medwed.txt` are **full-text extractions of copyrighted papers** — the stop the brief was written to produce. Thejus confirms they were his private copies for personal use; they must be gitignored and never published. |
| `2026-09-07_publication-list-independent-verification` | job_search / web | audit | **AUDITED ACCEPT** | `reports/2026-09-07_publication-list-independent-verification_REPORT.md` | **ACCEPT.** Boundary clean (only the report created; application folder mtimes unchanged). All 6 works AGREE with the leader's list. **It found two leader errors, both real:** the A&A 2019 paper does have a DOI (`10.1051/0004-6361/201935760`) and the JPCS 2017 paper does too (`10.1088/1742-6596/875/11/102022`); the leader had recorded '(none recorded)' for both. **Leader re-verified both against Crossref himself** — A&A: Mahajan author 8 of 9; JPCS 875: Mahajan author 1 of 23. Both now on the CV. It also found the **erratum** to the A&A paper (A&A 636, C2, 2020) and correctly judged it not a separate publication, and correctly excluded the thesis, 2 conference presentations and 1 abstract. Both disputed website citations confirmed bad. Classification: **4 JOURNAL, 2 PROCEEDINGS** — leader acted on it and split the CV section under two headings. **Two caveats: (1) INDEPENDENCE IS PARTIAL** — it declared, honestly and unprompted, that the whole brief file including sealed Appendix A was in context from the start, so anchoring cannot be excluded; the 6/6 agreement is therefore weaker evidence than it looks, though the two DOI discoveries and the erratum are *not* in Appendix A and are genuine independent work. **(2) Minor scope drift** — §4 ruled on how the CV should word the heading after the brief said that was the leader's call; it gave the counts as asked, then editorialised. **Leader error to log:** the brief's source-file list omitted `launoy2017.bibtex` (leader's `ls *.bib` missed the `.bibtex` extension); the worker found it anyway. |
